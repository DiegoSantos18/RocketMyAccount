...
export class NotificationService {
  ...
  success(message: string) {
    this.openSnackBar(`✔️ ${message}`, '', 'success-snackbar', 3000, 'right', 'bottom');
  }
  ...
  error(message: string) {
    this.openSnackBar(`❌ ${message}`, '', 'error-snackbar', 3000, 'right', 'bottom');
  }
   ...
   * @param message Message to display
   * @example
   * this.notificationService.alert("confirm alert");
   */
   alert(message: string) {
    this.openSnackBar(`⚠️ ${message}`, '', 'alert-snackbar', 3000, 'right', 'bottom');
  }
  ...
  */
  alertDialog(message: string, title = 'Notice', okCallback: () => void = () => { }) {
